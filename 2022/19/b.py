...
def shouldDo(ores, blue, robots, t):
    should = set(["geode"])
    for (_, costs) in blue.items():
        for (typ, co) in costs:
            if robots[typ] * t + ores[typ] < t * co:
                should.add(typ)
    return should

def canGeode(ores, blue):
    for (typ, co) in blue["geode"]:
        if co > ores[typ]:
            return False
    return True
...
def getNumGeos(blue, t, robots, ores, brain):
    ...
    if key in brain:
        return brain[key]
    if canGeode(ores, blue):
        addTime(ores, robots, 1)
        doRobot(ores, blue, "geode", 1)
        robots["geode"] += 1
        ans = getNumGeos(blue, t - 1, robots, ores, brain)
        robots["geode"] -= 1
        doRobot(ores, blue, "geode", -1)
        addTime(ores, robots, -1)
        return ans
    curMax = 0
    for rob in shouldDo(ores, blue, robots, t):
        maxT = 0
        for (typ, co) in blue[rob]:
            if robots[typ] == 0:
                maxT = None
        ...
        if maxT is not None and maxT + 1 <= t:
...
def main():
    ...
    for i in range(3):
        ...
        for typ in allOres:
            ...
            robos[typ] = 0
        robos["ore"] = 1
        tot *= getNumGeos(blue, 32, robos, ores, dict())
    print(tot)
