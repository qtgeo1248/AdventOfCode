...
def deepCopy(snailSum):
    if type(snailSum) is int:
        return snailSum
    return [deepCopy(snailSum[0]), deepCopy(snailSum[1])]
...
def main():
    ...
    for i in range(0, len(addends)):
        for j in range(0, len(addends)):
            if i != j:
                left = deepCopy(addends[i])
                right = deepCopy(addends[j])
                mag = magnitude(reduced([left, right]))
                largestMag = mag if mag > largestMag else largestMag

    print("Answer: " + str(largestMag))
    f.close()
